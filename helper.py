...
def load_features_challenge(
    ...
    features_df = features_df.sort_index(axis=1)
    print(f"Loaded n = {len(features_df)} feature vectors")
    return features_df, df_labels, features_df.columns.tolist()
...
def get_challenge_data(preprocess=True) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray, list[str]]:
    ...
    X_train,X_challenge,feature_names = project1.transform(X_train,X_challenge)
